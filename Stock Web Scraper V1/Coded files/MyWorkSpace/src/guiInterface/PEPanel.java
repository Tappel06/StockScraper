...
public class PEPanel extends JPanel implements ActionListener{
	...
	public static JLabel currentPEIndex;
	...
	PEPanel(){
		...
		scrapeDisplayPanel.add(HTMLLabelPanel);
		currentSource = new JLabel(DataVariables.PEHTMLSourceCode);
		...
		currentPEIndex = new JLabel(DataVariables.indexPEString);
		currentPEIndex.setForeground(Color.green);
		HTMLBlankPanel.add(currentPEIndex);
		...
		minimumLabel = new JLabel(DataVariables.PEMinimum);
		...
		maxLabel = new JLabel(DataVariables.PEMaximum);
		...
		if(DataVariables.PEWebOption == 'A') {
			addOption0RadioButton.setSelected(true);
		} else if(DataVariables.PEWebOption == 'B') {
			addOption1RadioButton.setSelected(true);
		} else if(DataVariables.PEWebOption == 'C') {
			addOption2RadioButton.setSelected(true);
		}
		if(addOption0RadioButton.isSelected() == true) {
			webLabel.setText(DataVariables.PEWebsite1 + "(ticker)");
		}
		if(addOption1RadioButton.isSelected() == true) {
			webLabel.setText(DataVariables.PEWebsite1 + "(ticker)" + DataVariables.PEWebsite2);
		}
		if(addOption2RadioButton.isSelected() == true) {
			webLabel.setText(DataVariables.PEWebsite1 + "(ticker)" + DataVariables.PEWebsite2 + "(ticker)" + DataVariables.PEWebsite3);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==listARadioButton) {
			DataVariables.tickerPEList = 'A';
			FileStorage.tickerPEListSave();
			
		}
		
		if(e.getSource()==listBRadioButton) {
			DataVariables.tickerPEList = 'B';
			FileStorage.tickerPEListSave();
			
		}
		
		if(e.getSource()==listCRadioButton) {
			DataVariables.tickerPEList = 'C';
			FileStorage.tickerPEListSave();
			
		}
		
		if(e.getSource()==addOption0RadioButton) {
			DataVariables.PEWebOption = 'A';
			FileStorage.PEWebOptionSave();
		}
		
		if(e.getSource()==addOption1RadioButton) {
			DataVariables.PEWebOption = 'B';
			FileStorage.PEWebOptionSave();
		}
		
		if(e.getSource()==addOption2RadioButton) {
			DataVariables.PEWebOption = 'C';
			FileStorage.PEWebOptionSave();
		}
		...
		if(e.getSource()==changeButton) {
			...
			if(addOption0RadioButton.isSelected() == true) {
				DataVariables.PEWebsite1 = mainWebsiteCombo.getSelectedItem().toString();
				webLabel.setText(DataVariables.PEWebsite1 + "(ticker)");
				FileStorage.PEWebConstructorSave1();
			}
			
			if(addOption1RadioButton.isSelected() == true) {
				DataVariables.PEWebsite1 = mainWebsiteCombo.getSelectedItem().toString();
				DataVariables.PEWebsite2 = alternative1Combo.getSelectedItem().toString();
				webLabel.setText(DataVariables.PEWebsite1 + "(ticker)" + DataVariables.PEWebsite2);
				FileStorage.PEWebConstructorSave2();
			}
			
			if(addOption2RadioButton.isSelected() == true) {
				DataVariables.PEWebsite1 = mainWebsiteCombo.getSelectedItem().toString();
				DataVariables.PEWebsite2 = alternative1Combo.getSelectedItem().toString();
				DataVariables.PEWebsite3 = alternative1Combo.getSelectedItem().toString();
				webLabel.setText(DataVariables.PEWebsite1 + "(ticker)" + DataVariables.PEWebsite2 + "(ticker)" + DataVariables.PEWebsite3);
				FileStorage.PEWebConstructorSave3();
			}
		...
		if(e.getSource()==HTMLChangeButton) {
			DataVariables.PEHTMLSourceCode = HTMLField.getText();
			currentSource.setText(HTMLField.getText());
			DataVariables.indexPE = Integer.parseInt(index.getText());
			currentPEIndex.setText(index.getText());
			FileStorage.PEHTMLSourceSave();
			FileStorage.PEIndexSourceSave();
	}
		
		if(e.getSource()==setLimitsButton) {
			minimumLabel.setText(minimumTextField.getText());
			maxLabel.setText(maxTextField.getText());
			DataVariables.PEMinimum = minimumTextField.getText();
			DataVariables.PEMaximum = maxTextField.getText();
			FileStorage.PEParametersSave();
		}
	}
}
